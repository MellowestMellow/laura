...
export class ReporteService {
  ...
  async exportToPDFConEtapasFlexibleFormatted(dt: any, columnasBase: { campo: string; nombre: string }[], fileName: string = 'reporte'
  ) {
    const data = dt.filteredValue ?? dt.value;
    const etapasUnicas = new Map<number, string>();
    ...
    const drawHeader = () => {
      ...
      doc.setTextColor(0);
      doc.text('Reporte de Solicitudes', pageWidth / 2, 43, {
        align: 'center',
      }); // Centrado
