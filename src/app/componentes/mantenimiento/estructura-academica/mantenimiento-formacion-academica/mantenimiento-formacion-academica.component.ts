    ...
    this.srvMantenimientoGradoService.deleteGrado(idGrado).subscribe(
      (res: any) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Operación Exitosa',
          detail: res.message || 'Grado Académico eliminado exitosamente',
          life: 3000
        });
      },
      (error: any) => {
        this.isLoadingEliminar = false;
         this.closableDialog = true;
        // Mostrar mensaje de error
        const msg = error.error?.mensaje || 'Error al eliminar el grado académico';
        this.messageService.add({
          severity: 'error',
          summary: 'Operación Fallida',
          detail: msg,
    ...
    this.srvMantenimientoTituloAcademicoService.deleteTituloAcademico(idTituloAcademico).subscribe(
      (res: any) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Operación Exitosa',
          detail: res.message || 'Título Académico eliminado exitosamente',
          life: 3000
        });
      },
      (error: any) => {
        ...
        this.closableDialog = true;
        // Mostrar mensaje de error
        const msg = error.error?.mensaje || 'Error al eliminar el título académico';
        this.messageService.add({
          severity: 'error',
          summary: 'Operación Fallida',
          detail: msg,
