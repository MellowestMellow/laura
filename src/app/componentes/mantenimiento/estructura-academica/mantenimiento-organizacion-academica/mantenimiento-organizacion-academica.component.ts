...
export class MantenimientoOrganizacionAcademicaComponent {
  ...
  cargarCampus(): void {
    ...
    this.srvMantenimientoCampusService.getCampus().subscribe(
      ...
      (error) => {
        this.loadingCampus = false;  // Detener el estado de carga
        this.showErrorMessage(error);  // Mostrar mensaje de error (puedes mejorar la visualización)
      }
    );
  }
  ...
  actualizarCampus() {
    ...
    const formdata = this.ActualizarCampusForm.value;
    ...
    this.srvMantenimientoCampusService.putCampus(formdata).subscribe(
      (res: any) => {
        ...
        this.ActualizarCampusForm.reset();
    ...
    this.srvMantenimientoCampusService.deleteCampus(idCampus).subscribe(
      (res: any) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Operación Exitosa',
          detail: res.message || 'Campus Académico eliminado exitosamente',
          life: 3000
        });
      },
      (error: any) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Operación Fallida',
          detail: error.error?.mensaje || 'Error al eliminar el campus académico',
          life: 3000
        });
      }
    );
    ...
    this.srvMantenimientoCampusService.deleteCampusDepartamento(idCampusDepartamento).subscribe(
      ...
      (error: any) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: error.error?.mensaje || error.message || 'No se pudo eliminar el departamento del campus.',
          life: 3000
        });
      }
    );
    ...
    this.srvMantenimientoCampusService.putCampusTituloAcademico(formdata).subscribe(
      (res: any) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Operación Exitosa',
          detail: res.message || 'Título Académico actualizado exitosamente en el Campus Académico',
          life: 3000
        });
      },
      (error: any) => {
