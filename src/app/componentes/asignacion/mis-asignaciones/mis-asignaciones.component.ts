...
export class MisAsignacionesComponent implements OnInit {
  ...
  private evaluarTiposConArchivo(asignacion: Asignacion): EvaluacionResult {
    ...
    const estado = roles.some(r => [3, 4].includes(r)) ? 29 : 20;
    return {
      idEstado: estado,
  ...
  private enviarEvaluacion(data: EvaluacionResult): void {
    ...
    if (!this.esEspecialActual && this.selectedFiles.length > 0) {
  ...
  debeMostrarEvaluacion(rowData: any): boolean {
    ...
    if (etapaActual === 3) {
      const condicion1 = (etapa2?.idEstadoSolicitud === 26 && etapa3?.idEstadoSolicitud === 16) || etapa2?.idEstadoSolicitud === 5;
