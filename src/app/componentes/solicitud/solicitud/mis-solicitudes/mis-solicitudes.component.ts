...
@Component({
  ...
  imports: [PrimeNGImports, AngularImports, HistorialModalComponent, ContadorComponent],
    ...
    this.srvSolicitud.getSolicitudUsuario().subscribe(
      (data: any[]) => {
        this.solicitud = data.map((item: any, index: number) => {
          ...
          const archivosEntrada =
            item.archivosRecientes?.filter((a: any) => a.idTipoArchivo === 1) || [];
          const archivosSalida =
            item.archivosRecientes?.filter((a: any) => a.idTipoArchivo === 2) || [];
