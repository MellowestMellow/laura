...
export class UsuarioInternoComponent implements OnInit {
  ...
  cargarUsuarios(): void {
    ...
    this.UsuarioService.getUsuarioPersona().subscribe(
      (response: any) => {
        const rolesExcluidos = [1, 2, 11, 13, 17, 18, 19];

        const usuariosFiltrados = response.filter((usuario: any) => {
          // Excluir si idRoles es null o undefined
          if (!usuario.idRoles) return false;

          const rolesUsuario = usuario.idRoles.split(", ").map((rol: string) => Number.parseInt(rol.trim(), 10));
          return !rolesUsuario.some((rol: number) => rolesExcluidos.includes(rol));
        });
